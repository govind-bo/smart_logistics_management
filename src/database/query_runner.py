...
def run_sql_file(folder: str, sql_file: str, params: dict | None = None) -> pd.DataFrame:               # NEEDS CLEANUP------------------------------------
    """
...
def fetch_filtered_data(folder: str, sql_file: str, params: dict) -> pd.DataFrame:
    """
    ...
    if not params or "start_date" not in params:
        # If no page filters are passed, execute the lookup query directly
        with engine.connect() as conn:
            return pd.read_sql(text(query_string), con=conn)
